...
const event = (type, properties) =>
  stream.write(
    `data: ${JSON.stringify({ type, properties: { sessionID: 'oc-session', ...properties } })}\r\n\r\n`
  )
...
const server = createServer(async (req, res) => {
  ...
  if (process.env.ATTACHMENT_CAPTURE && body.parts)
    appendFileSync(process.env.ATTACHMENT_CAPTURE, JSON.stringify(body) + '\n')
  const json = (value) => {
  ...
  if (path === '/session/oc-session/prompt_async') {
    ...
    event('message.updated', {
      info: { sessionID: 'oc-session', id: `m${turn}`, role: 'assistant' }
    })
    event('message.part.updated', {
      part: {
        ...
        type: 'tool',
        tool: 'read',
        state: { status: 'running', input: { path: 'file' } }
      }
    })
    event('message.part.updated', {
      part: {
        ...
        type: 'tool',
        tool: 'read',
        state: { status: 'completed', input: { path: 'file' }, output: 'read' }
      }
    })
    event('message.part.updated', {
      part: {
  ...
  if (path === '/permission/approval/reply') {
    ...
    json(true)
    event('message.updated', {
      info: { sessionID: 'oc-session', id: 'decision', role: 'assistant' }
    })
    event('message.part.updated', {
      part: {
        sessionID: 'oc-session',
  ...
  if (path === '/question/question/reply') {
    ...
    event('message.part.updated', {
      part: {
        ...
        id: 'answer',
        type: 'text',
        text: JSON.stringify(body.answers)
      }
    })
    finish()
    return
  }
  if (path === '/session/oc-session/abort') {
    json(true)
    finish()
    return
  }
