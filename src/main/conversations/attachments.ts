...
export async function readBounded(path: string, limit: number): Promise<Buffer> {
  ...
  try {
    ...
    if (info.size > limit)
      throw new Error(
        'File is too large for a preview or direct image. Send it as a file reference.'
      )
...
/** Managed copies have conversation lifetime, unlike the terminal's seven-day drop cache. */
...
/** Validate before acceptance. Paths are data, never shell commands. */
