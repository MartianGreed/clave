...
export interface ClaveGroupData {
  ...
  prompt?: string
  sessions: { cwd: string; name: string; claudeMode: boolean; antigravityMode: boolean; codexMode: boolean; piMode?: boolean; claudeAgentsMode?: boolean; dangerousMode: boolean; prompt?: string; rootSession?: boolean; /** The account (subscription) by label, or `any` (ADR 0002). Routes the session; drives nothing, so it is not elevated. */ account?: string; /** @deprecated legacy alias for antigravityMode, read for back-compat */ geminiMode?: boolean }[]
