...
export async function run(t) {
  ...
  seedWorkspaces(userData, {
    workspaces: [
      { id: 'ux-workspace', name: 'Conversation UX', rootDir: root, profileFile: null, createdAt: 1 }
    ],
  ...
  try {
    ...
    await app.evaluate(({ ipcMain, BrowserWindow }, { root, review, notice }) => {
      ...
      for (const provider of ['claude', 'pi']) {
        ...
        fixture.snapshots[id] = {
          session: {
            id, provider, cwd: root, workspaceId: 'ux-workspace',
            title: `${provider}: transcript follow behavior`,
            createdAt: '', updatedAt: '', status: 'idle',
            capabilities: {
              permissions: provider !== 'pi', questions: true, resume: true,
              ...(provider === 'pi' ? { notice } : {})
            }
          },
          sequence: 0,
          entries: [
            { kind: 'message', id: 'opening', role: 'user', text: 'Review the conversation panel. Keep my place while I read earlier output, and preserve drafts when I switch sessions.' },
            ...Array.from({ length: 9 }, (_, i) => [
              { kind: 'message', id: `review-${i}`, role: 'assistant', text: `### Review pass ${i + 1}\n\n${review}` },
              { kind: 'tool', id: `tool-${i}`, name: 'Read file', input: 'src/renderer/src/components/terminal/ConversationPanel.tsx', output: `Pass ${i + 1}: inspected the scroll effect and composer state.\nNo files changed.`, status: 'completed' }
            ]).flat(),
            { kind: 'message', id: 'stream', role: 'assistant', text: '## Verification plan\n\nI will exercise draft isolation, keyboard input, and scroll following in real Electron.' }
          ],
          requests: []
        }
      }
      ...
      fixture.emit = (sessionId, event) => {
        ...
        if (event.type === 'request-resolved') {
          snapshot.requests = snapshot.requests.filter((request) => request.id !== event.requestId)
        }
        if (event.type === 'turn-end') snapshot.session.status = 'idle'
        if (event.type === 'status') snapshot.session.status = event.status
        const envelope = {
          sessionId, sequence: ++snapshot.sequence, timestamp: new Date().toISOString(), event
        }
      ...
      ipcMain.handle('conversation:command', (_event, command) => {
        ...
        if (command.type === 'send') {
          ...
          if (fixture.holdNext) {
            ...
            return new Promise((resolve) => { fixture.pending = resolve })
          }
        }
        if (command.type === 'respond') {
          fixture.emit(command.sessionId, { type: 'request-resolved', requestId: command.response.requestId })
        }
    ...
    const calls = (type) => app.evaluate(
      (_electron, type) => globalThis.__conversationUX.calls.filter((call) => call.type === type), type
    )
    const emit = (event, sessionId = CLAUDE) => app.evaluate(
    ...
    t.check('Enter sends exactly one multiline message', await until(async () => {
      const sent = await calls('send')
      return sent.length === 1 && sent[0].text === 'First line\nSecond line'
    }))
    t.check('accepted message clears the submitted draft', await until(async () => await composer.inputValue() === ''))
    ...
    await composer.dispatchEvent('keydown', {
      key: 'Enter', code: 'Enter', keyCode: 229, which: 229, isComposing: true, bubbles: true
    })
    ...
    await callMcp(app, 'focus', { sessionId: CLAUDE })
    t.equal('Claude draft survives switching sessions', await composer.inputValue(), 'Claude draft: inspect the reducer')
    await callMcp(app, 'focus', { sessionId: PI })
    t.equal('Pi keeps its independent draft', await piComposer.inputValue(), 'Pi draft: add the regression')
    await win.reload()
    await win.waitForLoadState('domcontentloaded')
    await win.locator('[data-testid="conversation-panel"]').first().waitFor()
    await callMcp(app, 'focus', { sessionId: CLAUDE })
    ...
    t.check('fixture holds the send acknowledgment', await until(() => app.evaluate(() => Boolean(globalThis.__conversationUX.pending))))
    await composer.fill('Next draft: inspect the failure output')
    await emit({ type: 'request', request: { id: 'approval', kind: 'permission', title: 'Run the focused test?', description: 'npm run test:e2e -- conversation-ux' } })
    await panel.getByRole('button', { name: 'Allow', exact: true }).click()
    t.check('permission remains usable during pending send', (await calls('respond')).some((call) => call.response.requestId === 'approval' && call.response.decision === 'allow'))
    await emit({ type: 'request', request: { id: 'target', kind: 'question', title: 'Which test target?', choices: ['Real Electron', 'Unit only'] } })
    await panel.getByRole('button', { name: 'Real Electron', exact: true }).click()
    t.check('question remains usable during pending send', (await calls('respond')).some((call) => call.response.requestId === 'target' && call.response.answer === 'Real Electron'))
    await app.evaluate(() => {
    ...
    await win.waitForTimeout(150)
    t.equal('late send acknowledgment preserves the next draft', await composer.inputValue(), 'Next draft: inspect the failure output')
    ...
    await emit({ type: 'text-delta', messageId: 'stream', text: '\n\nNew output while the reader checks an earlier review pass.' })
    await panel.getByText('New output while the reader checks an earlier review pass.', { exact: true }).waitFor({ state: 'attached' })
    await win.waitForTimeout(150)
    const after = await article.evaluate(scrollMetrics)
    t.check('streaming delta preserves the older reading position', Math.abs(after.top - before.top) < 2, { before, after })
    ...
    if (await jump.isVisible()) {
      await jump.click()
      t.check('Jump to latest reaches the transcript bottom', await until(async () => (await article.evaluate(scrollMetrics)).remaining < 4))
      const attachedHeight = (await article.evaluate(scrollMetrics)).height
      await emit({ type: 'text-delta', messageId: 'stream', text: '\n\n' + 'The follow-up regression passed.\n\n'.repeat(15) })
      t.check('new deltas follow after reattaching', await until(async () => {
    ...
    t.check('Stop interrupts the conversation', (await calls('interrupt')).some((call) => call.sessionId === CLAUDE))
    await panel.getByRole('button', { name: 'Session details', exact: true }).click()
    t.check('capabilities are available in Session details', await win.getByTestId('conversation-capabilities').getByText(/Permission review available/).isVisible())
    ...
    for (const theme of ['dark', 'light', 'coffee', 'charcoal']) {
      ...
      t.equal(`${theme} theme is applied`, await win.evaluate(() => document.documentElement.dataset.theme), theme)
      await panel.locator('summary').first().click()
      const fit = await panel.evaluate((element) => {
        const box = element.getBoundingClientRect()
        const textarea = element.querySelector('textarea').getBoundingClientRect()
        const articles = [...element.querySelectorAll('article')].map((article) => article.getBoundingClientRect())
        ...
        return {
          ...
          messagesFit: articles.every((article) => article.left >= box.left && article.right <= box.right + 1),
          noTranscriptOverflow: transcript !== null && transcript.scrollWidth <= transcript.clientWidth + 1,
          noPageOverflow: document.documentElement.scrollWidth <= innerWidth + 1
        }
      })
      t.check(`${theme} narrow layout fits messages and composer`, Object.values(fit).every(Boolean), fit)
      t.check(`${theme} tool details remain usable`, await panel.getByText('Pass 1: inspected the scroll effect and composer state.\nNo files changed.', { exact: true }).isVisible())
    ...
    if (existsSync(owner)) {
      ...
      t.check('test-owned daemon stops', await until(() => {
